...
XGB_PATH = Path(__file__).resolve().parent / "pitch_coach_xgb.json"

_XGB_MODEL = None  # lazily loaded singleton


def _load_xgb():
    """Load the bundled XGBoost model once per warm function instance."""
    global _XGB_MODEL
    if _XGB_MODEL is None:
        if not XGB_PATH.exists():
            raise HTTPException(
                status_code=500,
                detail="XGBoost model missing. Run train_xgboost.py and redeploy.",
            )
...
def _classify(embedding: list, features: dict) -> dict:
    """Score BERT embedding + linguistic features with the bundled XGBoost model."""
    model = _load_xgb()
    ling = features_to_vector(features)
    X = np.array(embedding + ling, dtype="float32").reshape(1, -1)
    if X.shape[1] != EMBEDDING_DIM + len(ling):
        raise HTTPException(status_code=500, detail="Feature vector size mismatch.")

    good_prob = float(model.predict_proba(X)[0][1])
