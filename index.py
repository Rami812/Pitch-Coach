...
# All shared helpers live at the project root and are bundled via
# vercel.json `includeFiles`. Keeping them out of /api means Vercel builds
# exactly ONE serverless function (api/index.py) and imports resolve here.
...
# Pure-Python XGBoost evaluator (model_predict.py reads model_trees.json) — no
# xgboost / scipy / numpy at runtime.
try:
    from model_predict import score as _xgb_score
except Exception:
    _xgb_score = None  # surfaced at request time with a clear error

# Frontend HTML embedded as a Python module so import tracing carries it.
