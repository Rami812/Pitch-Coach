...
def positive_prob(score_output) -> float:
    """
    Normalize an m2cgen `score()` result into P(good).

    m2cgen may return a scalar (probability of the positive class) or a list of
    per-class scores ([P(bad), P(good)] for binary). Handle both and clamp 0–1.
    """
...
def pool_hf_embedding(hf_response) -> list:
    ...
    Pure Python (no numpy) so the Vercel serverless function stays dependency-light.
    """
    def depth(x) -> int:
        d = 0
